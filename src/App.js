...
// As importações do Firebase devem vir do seu arquivo de configuração
// Exemplo: import { db, auth } from './firebase/config';
...
// ===================================================================================
//  2. HOOKS PERSONALIZADOS (CORRIGIDOS)
// ===================================================================================

// --- hooks/useAuth.js (Versão Corrigida) ---
const useAuth = () => {
    const [currentUser, setCurrentUser] = useState(null);
    // NOVO: Estado para sabermos quando a autenticação inicial está carregando
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        const unsub = onAuthStateChanged(auth, async (user) => {
            if (user) {
                // ALTERADO: Adicionado try...catch para capturar erros de permissão
                try {
                    ...
                    console.error("Erro ao buscar dados do usuário (verifique as regras de segurança do Firestore):", error);
                    // Se houver erro, desloga para evitar estado inconsistente
                    await signOut(auth);
                    setCurrentUser(null);
                }
            } else {
            ...
            // NOVO: Finaliza o carregamento após a verificação inicial
            setLoading(false);
        });
    ...
    // Retorna também o estado de loading
    return { currentUser, loading, login, logout };
};


// --- hooks/useCondoData.js (Versão Corrigida) ---
const useCondoData = () => {
    const [allData, setAllData] = useState({});
    // NOVO: Estados de loading e erro para melhor controle da UI
    const [dataLoading, setDataLoading] = useState(true);
    const [dataError, setDataError] = useState(null);

    useEffect(() => {
        const unsub = onSnapshot(collection(db, 'lotes'), 
            ...
            (snapshot) => {
                ...
                setDataError(null); // Limpa erros anteriores
            }, 
            // 2. ALTERADO: Callback de erro (ESSENCIAL)
            (err) => {
                console.error("Erro no listener do Firestore (lotes - verifique as regras de segurança):", err);
                setDataError("Não foi possível carregar os dados dos lotes.");
                setDataLoading(false);
            }
        );
    ...
    const updateAllData = async (newData, loteIdAlterado) => {
        // ALTERADO: Adicionado try...catch para a operação de escrita
        try {
            await setDoc(doc(db, 'lotes', loteIdAlterado), { registros: newData[loteIdAlterado] });
            // Não é mais necessário o setAllData aqui, pois o onSnapshot já fará a atualização
            return { success: true };
        } catch (err) {
            console.error("Erro ao atualizar o lote:", err);
            return { success: false, error: "Permissão negada para atualizar os dados." };
        }
    };

    // Retorna os novos estados
    return { allData, dataLoading, dataError, updateAllData };
};


// ===================================================================================
//  3. CONTEXTO DA APLICAÇÃO (ATUALIZADO)
// ===================================================================================
...
export const AppProvider = ({ children }) => {
    ...
    const condoDataHook = useCondoData();
    ...
    // ALTERADO: Une os valores dos hooks para prover ao contexto
    const value = { ...authHook, ...condoDataHook, scriptsLoaded };
...
// ===================================================================================
//  4. COMPONENTES DE INTERFACE (UI) - SEM ALTERAÇÕES NECESSÁRIAS
// ===================================================================================
...
// ... (Todos os outros componentes permanecem iguais: LoteListModal, DashboardView, RegistroForm, etc.)
...
    const { allData, dataLoading, dataError } = useAppContext();
    const [modalInfo, setModalInfo] = useState({ show: false, title: '', lotes: [] });
    
    // Mostra feedback de loading ou erro
    if (dataLoading) return <p className="text-center text-gray-500">Carregando dados do dashboard...</p>;
    if (dataError) return <p className="text-center text-red-600 bg-red-100 p-4 rounded-lg">{dataError}</p>;
...
        data.anomalias.baixas = data.ranking.filter(r => r.consumo > 0 && r.consumo < thresholdBaixo).slice(0, 5);
        return data;
    }, [allData]);
...
// ... (O restante dos componentes: RegistroForm, HistoricoTable, AnaliseView, LoteManager, AppView) ...
...
const HistoricoTable = ({ registros, onEdit, onDelete, isAdmin }) => {
    const sortedRegistros = useMemo(() => 
        [...registros].sort((a, b) => new Date(`${b.mesAno}-01`) - new Date(`${a.mesAno}-01`))
    , [registros]);

    if (sortedRegistros.length <= 20) {
        return (
...
        const r = sortedRegistros[index];
        return (
            <div style={style} className="flex items-center hover:bg-gray-50/50">
                <div className="py-4 px-4 border-b border-gray-200 text-gray-700 w-1/4">{r.mesAno.split('-').reverse().join('/')}</div>
                <div className="py-4 px-4 border-b border-gray-200 w-1/4">
                    <span className="font-semibold text-gray-800">{r.consumo.toFixed(2)} m³</span>
                    <span className="block text-sm text-gray-500">{(r.consumo * 1000).toLocaleString('pt-BR')} litros</span>
                </div>
                <div className="py-4 px-4 border-b border-gray-200 text-gray-700 w-1/4">{r.custo.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</div>
                {isAdmin && (
                    <div className="py-4 px-4 border-b border-gray-200 text-sm space-x-4 w-1/4">
                        <button onClick={() => onEdit(r.id)} className="font-semibold text-gray-700 hover:text-black">Editar</button>
                        <button onClick={() => onDelete(r.id)} className="font-semibold text-red-600 hover:text-red-500">Remover</button>
                    </div>
                )}
...
        <div className="overflow-x-auto">
            <div className="flex bg-transparent">
                <div className="py-3 px-4 border-b border-gray-200 text-left text-sm font-semibold text-gray-500 w-1/4">Mês/Ano</div>
                <div className="py-3 px-4 border-b border-gray-200 text-left text-sm font-medium text-gray-500 w-1/4">Consumo</div>
                <div className="py-3 px-4 border-b border-gray-200 text-left text-sm font-medium text-gray-500 w-1/4">Custo (R$)</div>
                {isAdmin && <div className="py-3 px-4 border-b border-gray-200 text-left text-sm font-medium text-gray-500 w-1/4">Ações</div>}
            </div>
            <List
                height={400}
                itemCount={sortedRegistros.length}
                itemSize={65} // Altura da linha
                width="100%"
            >
                {Row}
            </List>
...
    const exportContentRef = useRef();
    const sortedRegistros = useMemo(() => 
        [...registros].sort((a, b) => new Date(`${a.mesAno}-01`) - new Date(`${b.mesAno}-01`))
    , [registros]);

    if (sortedRegistros.length < 1) return <div className="text-center py-12 px-4 border-dashed border-2 border-gray-300 rounded-lg"><p className="text-gray-500">Adicione pelo menos um registro para gerar um comparativo.</p></div>;
    
    const chartData = {
        labels: sortedRegistros.map(r => {
...
                data: sortedRegistros.map(r => r.consumo * 1000),
                backgroundColor: 'rgba(59, 130, 246, 0.7)', // blue-500
                borderColor: 'rgba(29, 78, 216, 1)', // blue-800
                borderWidth: 1,
                borderRadius: 4
            },
            {
                label: 'Limite 15.000 litros',
                data: Array(sortedRegistros.length).fill(15000),
                type: 'line',
...
        const ultimo = sortedRegistros[sortedRegistros.length - 1]; let msg = ''; let className = 'mt-4 text-center p-4 rounded-lg ';
        if (sortedRegistros.length === 1) { msg = `Primeiro registro: <strong>${ultimo.consumo.toFixed(2)} m³</strong> (${(ultimo.consumo * 1000).toLocaleString('pt-BR')} litros).`; className += 'bg-gray-100 text-gray-800'; }
        else { const penultimo = sortedRegistros[sortedRegistros.length - 2]; const diferenca = ultimo.consumo - penultimo.consumo; const diferencaLitros = Math.abs(diferenca * 1000).toLocaleString('pt-BR');
...
    const registros = useMemo(() => selectedLote ? (allData[selectedLote] || []) : [], [allData, selectedLote]);
...
            const newRecord = { id: `${Date.now()}`, mesAno, consumo, custo, ...parsedData }; // ID como string
...
    const handleExportCSV = () => {
        const sortedRegistros = [...registros].sort((a, b) => new Date(`${a.mesAno}-01`) - new Date(`${b.mesAno}-01`));
        if(sortedRegistros.length === 0) { alert("Nenhum registro para exportar."); return; }
...
        const rows = sortedRegistros.map(r => [ `"${r.mesAno.split('-').reverse().join('/')}"`, r.leituraAnterior, r.leituraAtual, r.consumo.toFixed(2), (r.consumo * 1000).toFixed(0), r.tarifa.toFixed(2), r.custo.toFixed(2) ]);
...
// --- Componente Raiz da Aplicação (ATUALIZADO) ---
...
const Root = () => {
    // ALTERADO: Pega 'currentUser' e 'loading' do contexto
    const { currentUser, loading } = useAppContext();

    // ALTERADO: Mostra uma mensagem de carregamento enquanto a autenticação é verificada
    if (loading) {
        return (
